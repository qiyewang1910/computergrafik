...
public class A01 {
  ...
  public static void main(String[] args) {
    ...
    ConstantColor magentaConstant = new ConstantColor(magenta);

    DiscLiveStart drawer = new DiscLiveStart();
    drawer.drawGridOfCircles(width, height, "6*6_circle", 6, 6, magenta);
  } 

  public static class DiscLiveStart {
    public void drawGridOfCircles(int width, int height, String fileName, int rows, int cols, Color circleColor){
      ...
      for (int row = 0; row < rows; row++){
        for (int col=0; col<cols; col++){
          ...
          drawCircle(image, centerX, centerY, radius, circleColor, black); 
        }            
      }     
      // 在画完所有像素后，保存图片
